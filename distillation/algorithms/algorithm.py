...
from distillation.architectures.tools import split_network
...
class Algorithm:
    ...
    def init_all_networks(self):
        ...
        for key, val in networks_defs.items():
            ...
            force = val.get('force', False)
            if key=='student_net':
                self.networks['feature_extractor'], self.networks['classifier'] = self.init_network(
                def_file, net_opt, pretrained_path, key, force=force)
                if 'optim_params' in val and val['optim_params'] is not None:
                    self.optim_params['feature_extractor'] = val['optim_params']
                    self.optim_params['classifier'] = (
                        val['optim_params'].get('classifier_optim', None))
                else:
                    self.optim_params['feature_extractor'] = None
                    self.optim_params['classifier'] = None
            elif key=='teacher_net':
                self.networks['feature_extractor_target'], self.networks['classifier_target'] = self.init_network(
                    def_file, net_opt, pretrained_path, key, force=force)
                if 'optim_params' in val and val['optim_params'] is not None:
                    self.optim_params['feature_extractor_target'] = val['optim_params']
                    self.optim_params['classifier_target'] = (
                        val['optim_params'].get('classifier_optim', None))
                else:
                    self.optim_params['feature_extractor_target'] = None
                    self.optim_params['classifier_target'] = None
            else:
                self.optim_params[key] = (
                    val.get('optim_params', None))
    ...
    def init_network(self, net_def_file, net_opt, pretrained_path, key, force=False):
        ...
        if pretrained_path is not None and isinstance(pretrained_path, str):
            self.load_pretrained(network, pretrained_path, force=force)
        if key=='student_net' or key=='teacher_net':
            feat_net, clf_net = split_network(network, net_opt['extract_from'], net_opt['extract_after_relu'])
            network = [feat_net, clf_net]
            if pretrained_path is not None and isinstance(pretrained_path, list):
                self.load_pretrained(network[0], pretrained_path[0], force=force)
                self.load_pretrained(network[1], pretrained_path[1], force=force)
    ...
    def load_pretrained(self, network, pretrained_path, force=False):
        ...
        if network_name not in list(pretrained_model.keys()):
            network_name = 'model'
        if pretrained_model[network_name].keys() == network.state_dict().keys() or force:
            network.load_state_dict(pretrained_model[network_name], strict=False)
        else:
            self.logger.info(
            ...
            for pname, param in network.named_parameters():
                if pname in pretrained_model['network']:
                    ...
                    param.data.copy_(pretrained_model[network_name][pname])
    ...
    def init_optimizer(self, net, optim_opts, key):
        ...
        if optim_type == 'adam':
            weight_decay = (
                optim_opts.get('weight_decay', 0.0))
    ...
    def evaluate(self, dloader, idx=None, use_tensorboard=True):
    ...
    def keep_record_of_best_model(self, eval_stats, current_epoch):
        if self.keep_best_model_metric_name is not None:
            ...
            if metric_name not in eval_stats.values:
                raise ValueError(f'The provided metric {metric_name} for keeping the best '
