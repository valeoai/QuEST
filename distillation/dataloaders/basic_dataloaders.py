...
class OnlyImageDataloader:
    def __init__(
        self, dataset, batch_size, num_workers=4, train=True, epoch_size=None,
        old=False):
        self.dataset = dataset
        self.batch_size = batch_size
        self.num_workers = num_workers
...
class OnlyImageDataloaderForUnsupervisedConsistencyLoss:
    def __init__(
        self, dataset, batch_size, num_workers=4, train=True, epoch_size=None):
        self.dataset = dataset
        self.batch_size = batch_size
    ...
    def get_iterator(self):
        def load_fun_(idx):
            img_idx = idx % len(self.dataset)
            img = self.dataset.__getitem__(img_idx, transform_mode='test')[0]
            img_p = self.dataset.__getitem__(img_idx, transform_mode='train')[0]
            return img, img_p
