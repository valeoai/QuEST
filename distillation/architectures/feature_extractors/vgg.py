'''Ported and modified from https://github.com/HobbitLong/RepDistiller/blob/master/models/vgg.py'''
...
class VGG(nn.Module):
    ...
    def __init__(self, cfg, batch_norm=False, num_classes=1000, downscale=False):
        ...
        self.reshape = tools.Reshape(-1, cfg[3][-1])
    ...
    def forward(self, x, is_feat=False, preact=False):
        ...
        f4 = x
        x = self.pool4(x)
        x = self.reshape(x) #x = x.view(x.size(0), -1)
        f5 = x
        x = self.classifier(x)
...
def create_model(opt):
    ...
    return VGG(cfg=num_filters, batch_norm=True,
