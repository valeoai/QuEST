...
'''Source: https://github.com/HobbitLong/RepDistiller/blob/master/models/resnet.py'''
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, is_last=False, with_preact=False):
...
class ResNet(nn.Module):
    ...
    def __init__(self, depth, num_filters, block_name='BasicBlock', num_classes=10, with_preact=False, downscale=False):
        ...
        self.reshape = tools.Reshape(-1, num_filters[3])
    ...
    def _make_layer(self, block, planes, blocks, stride=1):
        ...
        layers = list([])
        layers.append(block(self.inplanes, planes, stride, downsample, is_last=(blocks == 1), with_preact=self.with_preact))
        self.inplanes = planes * block.expansion
        for i in range(1, blocks):
            layers.append(block(self.inplanes, planes, is_last=(i == blocks-1), with_preact=self.with_preact))
...
def create_model(opt):
    ...
    return ResNet(
        ...
        num_classes = num_classes, downscale=downscale)
