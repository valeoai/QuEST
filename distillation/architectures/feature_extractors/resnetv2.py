...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10, zero_init_residual=False, with_preact=False, downscale=False):
        ...
        self.reshape = tools.Reshape(-1, 512 * block.expansion)
...
def create_model(opt):
    depth = opt['depth']
    num_blocks = opt['num_filters']
    num_classes = opt['num_classes']
    downscale = False
    if 'downscale' in list(opt.keys()):
        downscale = opt['downscale']
    if depth<50:
        return ResNet(BasicBlock, num_blocks=num_blocks, num_classes = num_classes, downscale=downscale)
    else:
        return ResNet(Bottleneck, num_blocks=num_blocks, num_classes=num_classes, downscale=downscale)
